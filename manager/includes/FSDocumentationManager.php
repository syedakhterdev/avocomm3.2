...
@define( 'ERROR', 0 );
@define( 'SUCCESS', 1 );
@define( 'SECTION_TITLE', 'Manage Foodservice Documentations' );
@define( 'ENTITY', 'Foodservice Documentation' );

class FSDocumentationManager {
	function __construct( $db ) {
		$this->tableName = 'fs_documentation';
		$this->idField = 'id';
		$this->db = $db;
		$this->error = '';
	}
	...
	function add( $fs_program_id, $period_id, $title, $description, $image, $document, $document_type_id, $active ) {
		// clean up incoming variables for security reasons
		...
		$sql = "INSERT INTO $this->tableName SET
			date_created = CURDATE(), fs_program_id = ?, period_id = ?, title = ?, description = ?, image = ?, document = ?, document_type_id = ?, active = ?
			;";
		$new_id = $this->db->exec( $sql, array( $fs_program_id, $period_id, $title, $description, $image, $document, $document_type_id, $active ), true );
		if ( $new_id ) {
	...
	function update( $id, $fs_program_id, $period_id, $title, $description, $image, $document, $document_type_id, $active ) {
		// clean up incoming variables for security reasons
		$id = (int)$id;
	...
	function delete ( $id, $fs_program_id, $period_id ) {
		...
		if ( $id && $fs_program_id && $period_id ) {
			$sql = "DELETE FROM $this->tableName WHERE $this->idField = ? AND fs_program_id AND period_id = ? LIMIT 1;";
	...
	function getFS_DocumentationsCount() {
	...
	function getFS_Documentations( $offset, $rowsPerPage ) {
