...
@define( 'ERROR', 0 );
@define( 'SUCCESS', 1 );
@define( 'SECTION_TITLE', 'Manage Shopper Related Links' );
@define( 'ENTITY', 'Shopper Related Link' );

class ShopperRelatedLinkManager {
	function __construct( $db ) {
		$this->tableName = 'shopper_related_links';
		$this->idField = 'id';
		$this->db = $db;
		$this->error = '';
	}
	...
	function getByID ( $shopper_program_id, $period_id, $id ) {
		...
		if ( $id && $shopper_program_id && $period_id ) { // make sure a valid ID was passed in
	...
	function add( $period_id, $shopper_program_id, $title, $description, $image, $url, $sort ) {
		// clean up incoming variables for security reasons
		...
		$sql = "INSERT INTO $this->tableName SET
			period_id = ?, shopper_program_id = ?, title = ?, description = ?, image = ?, url = ?, sort = ?
			;";
		$new_id = $this->db->exec( $sql, array( $period_id, $shopper_program_id, $title, $description, $image, $url, $sort ), true );
		if ( $new_id ) {
	...
	function update( $id, $period_id, $shopper_program_id, $title, $description, $image, $url, $sort ) {
		// clean up incoming variables for security reasons
		$id = (int)$id;
		...
		if ( $id ) {
			...
			$sql .= " WHERE $this->idField = ? AND period_id = ? AND shopper_program_id = ? LIMIT 1;";
	...
	function getShopper_Related_LinksCount() {
	...
	function getShopper_Related_Links( $offset, $rowsPerPage ) {
	...
	function deleteImage( $id ) {
		...
		if ( $id ) {
			...
			if ( $stmt = $this->db->query( $sql, array( $id ) ) ) {
				if ( $row = $stmt->fetch() ) {
					if ( $row['image'] && file_exists( $_SERVER['DOCUMENT_ROOT'] . "/assets/shopper_related_links/" . $row['image'] ) )
						if ( unlink( $_SERVER['DOCUMENT_ROOT'] . "/assets/shopper_related_links/" . $row['image'] ) ) {
							return SUCCESS;
						} else {
							$this->error = 'Could not delete file from (/assets/shopper_related_links/) directory.';
							return ERROR;
						}
				} else {
