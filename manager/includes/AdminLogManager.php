...
@define( 'ERROR', 0 );
@define( 'SUCCESS', 1 );
@define( 'SECTION_TITLE', 'Manage Admin Activity Log' );
ini_set('display_errors', 1);
class AdminLogManager {
	function __construct( $db ) {
		$this->tableName = 'admin_activity_log';
		$this->idField = 'id';
		$this->db = $db;
		$this->error = '';
	}
	...
	function getActivity( $offset, $rowsPerPage, $filters = '' ) {
		...
		$sql = "SELECT a.*, CONCAT( b.first_name, ' ', b.last_name ) AS full_name, c.activity_type FROM $this->tableName a , Admins b, admin_activity_type c
			WHERE a.id > 0 AND a.user_id = b.id AND a.admin_activity_type_id = c.id $filters ORDER BY a.date_created DESC LIMIT $offset, $rowsPerPage;";
