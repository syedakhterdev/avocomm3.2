...
@define( 'ERROR', 0 );
@define( 'SUCCESS', 1 );
@define( 'SECTION_TITLE', 'Manage Foodservice Related Links' );
@define( 'ENTITY', 'Foodservice Related Link' );

class FSRelatedLinkManager {
	function __construct( $db ) {
		$this->tableName = 'fs_related_links';
		$this->idField = 'id';
		$this->db = $db;
		$this->error = '';
	}
	...
	function getByID ( $fs_program_id, $period_id, $id ) {
		...
		if ( $id && $fs_program_id && $period_id ) { // make sure a valid ID was passed in
	...
	function add( $period_id, $fs_program_id, $title, $description, $image, $url, $sort ) {
		// clean up incoming variables for security reasons

		if ( (int)$period_id < 0 || $period_id == '' ) $this->error = 'Please enter a valid Period.';
		if ( (int)$fs_program_id < 0 || $fs_program_id == '' ) $this->error = 'Please enter a valid Foodservice Program.';
		...
		$sql = "INSERT INTO $this->tableName SET
			period_id = ?, fs_program_id = ?, title = ?, description = ?, image = ?, url = ?, sort = ?
			;";
		$new_id = $this->db->exec( $sql, array( $period_id, $fs_program_id, $title, $description, $image, $url, $sort ), true );
		if ( $new_id ) {
	...
	function update( $id, $period_id, $fs_program_id, $title, $description, $image, $url, $sort ) {
		// clean up incoming variables for security reasons
		$id = (int)$id;
		...
		if ( $id ) {
			...
			$sql .= " WHERE $this->idField = ? AND period_id = ? AND fs_program_id = ? LIMIT 1;";
	...
	function getFS_Related_LinksCount() {
	...
	function getFS_Related_Links( $offset, $rowsPerPage ) {
	...
	function deleteImage( $id ) {
		...
		if ( $id ) {
			...
			if ( $stmt = $this->db->query( $sql, array( $id ) ) ) {
				if ( $row = $stmt->fetch() ) {
					if ( $row['image'] && file_exists( $_SERVER['DOCUMENT_ROOT'] . "/assets/fs_related_links/" . $row['image'] ) )
						if ( unlink( $_SERVER['DOCUMENT_ROOT'] . "/assets/fs_related_links/" . $row['image'] ) ) {
							return SUCCESS;
						} else {
							$this->error = 'Could not delete file from (/assets/fs_related_links/) directory.';
							return ERROR;
						}
				} else {
