            ...
            <div class="row row-offcanvas row-offcanvas-left">
                ...
                <div class="col main pt-5 mt-3">
                    ...
                    <ol class="breadcrumb bc-3">
                        ...
                        <li>&nbsp;/&nbsp;</li>
                        <li><strong>Shopper Programs</strong></li>
                    </ol>

                    <div class="row my-4 mgr_body shopper programs">
                                    ...
                                    <tbody>
                                        ...
                                        if ($conn->num_rows() > 0) {
                                            while ($row = $conn->fetch($result)) {
                                                echo '
									<tr>
										<td width="39">' . ( ( $row['image'] ) ? "<img src=\"/manager/timThumb.php?src=/assets/shopper_programs/" . $row['image'] . "&h=60&zc=1\" class=\"listing_image\">" : '' ) . '
											<a href="edit.php?id=' . $row['id'] . '">' . $conn->parseOutputString($row['title']) . '</a></td>
											<td align="center">' . date('m/d/Y', strtotime($row['start_date'])) . '</td>
											<td align="center">' . date('m/d/Y', strtotime($row['end_date'])) . '</td>
                        ...
                        <a href="index.php?active=' . $row['id'] . '&cur=' . (int)$row['active'] . '&page=' . $page . '" class="action_btn '.(($row['active'] == '1')?'active':'deactive').'" onClick="return confirm(' . "'Are you sure you want to change the active status of this item?'" . ');">'.(($row['active'] == '1')?'ON':'OFF').'</a>
										...
										<td align="center" class="listing_icons" width="16">
                                            ...
                                            echo "<td colspan=\"8\">No shopper programs found.</td>";
                                        }
                                        ?>
                                    </tbody>
                                </table>
