            ...
            <div class="row row-offcanvas row-offcanvas-left">
                ...
                <div class="col main pt-5 mt-3">
                    ...
                    <ol class="breadcrumb bc-3">
                        ...
                        <li>&nbsp;/&nbsp;</li>
                        <li><strong>Foodservice Programs</strong></li>
                    </ol>

                    <div class="row my-4 mgr_body foodservice programs">
                        <div class="col-lg-10 col-md-8">
                            ...
                            $rowsPerPage = 15;
                            $total_count = $FSProgram->getFS_ProgramsCount();
                            ...
                            <div class="table-responsive">
                                ...
                                <table class="table table-striped table-sm">
                                    ...
                                    <tbody>
                                        <?php
                                        $result = $FSProgram->getFS_Programs($conn->offset, $rowsPerPage);

                                        if ($conn->num_rows() > 0) {
                                            while ($row = $conn->fetch($result)) {
                                                echo '
                                                        <tr>
                                                            <td>' . ( ( $row['image'] ) ? "<img src=\"/manager/timThumb.php?src=/assets/fs_programs/" . $row['image'] . "&h=60&zc=1\" class=\"listing_image\">" : '' ) . '
                                                                <a href="edit.php?id=' . $row['id'] . '">' . $conn->parseOutputString($row['title']) . '</a></td>
                                                            <td style="text-align: center;">' . date('m/d/Y', strtotime($row['start_date'])) . '</td>
                                                            <td style="text-align: center;">' . date('m/d/Y', strtotime($row['end_date'])) . '</td>
                                                            <td style="text-align: center;">' . $row['sort'] . '</td>
                                                            <td align="center" class="listing_icons"><a class="action_btn '.(($row['active'] == '1')?'active':'deactive').'">'.(($row['active'] == '1')?'ON':'OFF').'</a></td>
                                            ...
                                            echo "<td colspan=\"8\">No food service programs found.</td>";
                                        }
                                        ?>
                                    </tbody>
                                </table>
