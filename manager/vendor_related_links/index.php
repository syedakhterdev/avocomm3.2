...
if ($del_id) {
    ...
    if ($token != '' && $_POST['token'] == $token) {
        if (!$VendorRelatedLink->delete($del_id)) {
            ...
            <div class="row row-offcanvas row-offcanvas-left">
                ...
                <div class="col main pt-5 mt-3">
                    ...
                    <ol class="breadcrumb bc-3">
                        ...
                        <li>&nbsp;/&nbsp;</li>
                        <li><strong>Vendor Related Links</strong></li>
                    </ol>

                    <div class="row my-4 mgr_body vendor_related_link_index">
                        <div class="col-lg-10 col-md-8">
                            ...
                            <?php
                            //$sql = "SELECT COUNT(*) FROM vendor_related_links WHERE id > 0;";
                            $rowsPerPage = 15;
                            $total_count = $VendorRelatedLink->getVendor_Related_LinksCount();
                            ...
                            <div class="table-responsive">
                                ...
                                <table class="table table-striped table-sm">
                                    ...
                                    <tbody>
                                        <?php
                                        $result = $VendorRelatedLink->getVendor_Related_Links($conn->offset, $rowsPerPage);

                                        if ($conn->num_rows() > 0) {
                                            while ($row = $conn->fetch($result)) {
                                                echo '
                                                        <tr>
                                                            <td style="text-align: center;">' . $conn->parseOutputString($row['vendor_id']) . '</td>
                                                            <td>' . ( ( $row['image'] ) ? "<img src=\"/manager/timThumb.php?src=/assets/vendor_related_links/" . $row['image'] . "&h=60&zc=1\" class=\"listing_image\">" : '' ) . '<a href="edit.php?id=' . $row['id'] . '">' . $conn->parseOutputString($row['title']) . '</a></td>
                                                            ...
                                                            <td style="text-align: center;">
                                                                <form action="index.php?page=' . $page . '&criteria=' . $criteria . '" method="POST" onSubmit="return confirm(' . "'Are you sure you want to delete this item?'" . ');">
                                                                    ...
                                                                    <input type="submit" class="input_icon delete action_btn" value="DELETE">
                                                                </form>
                                                            </td>
                                                        </tr>';
                                            }
                                        } else {
                                            echo "<td colspan=\"6\">No vendor related links found.</td>";
                                        }
                                        ?>
                                    </tbody>
                                </table>
