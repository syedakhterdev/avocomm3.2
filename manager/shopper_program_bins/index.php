...
if ($del_id) {
    ...
    if ($token != '' && $_POST['token'] == $token) {
        if (!$ShopperProgramBin->delete($del_id)) {
            ...
            <div class="row row-offcanvas row-offcanvas-left">
                ...
                <div class="col main pt-5 mt-3">
                    ...
                    <ol class="breadcrumb bc-3">
                        ...
                        <li>&nbsp;/&nbsp;</li>
                        <li><strong>Kit Options</strong></li>
                    </ol>

                    <div class="row my-4 mgr_body shopper programs program_bin">
                        <div class="col-lg-10 col-md-8">
                            ...
                            <?php
                            //$sql = "SELECT COUNT(*) FROM shopper_program_bins WHERE id > 0;";
                            $rowsPerPage = 15;
                            $total_count = $ShopperProgramBin->getShopper_Program_BinsCount();
                            ...
                            <div class="table-responsive">
                                ...
                                <table class="table table-striped table-sm">
                                    ...
                                    <tbody>
                                        <?php
                                        $result = $ShopperProgramBin->getShopper_Program_Bins($conn->offset, $rowsPerPage);
                                        ...
                                        if ($conn->num_rows() > 0) {
                                            while ($row = $conn->fetch($result)) {
                                                echo '
                                                        <tr>
                                                            <td>
                                                                ' . ( ( $row['image'] ) ? "<img src=\"/manager/timThumb.php?src=/assets/shopper_program_bins/" . $row['image'] . "&h=60&zc=1\" class=\"listing_image\">" : '' ) . '
                                                                <a href="edit.php?id=' . $row['id'] . '">' . $conn->parseOutputString($row['title']) . '</a>
                                                            </td>
                                                            <td style="text-align: center;">' . $row['sort'] . '</td>
                                                            <td align="center" class="listing_icons">
                                                                <a class="action_btn '.(($row['active'] == '1')?'active':'deactive').'">'.(($row['active'] == '1')?'ON':'OFF').'</a>
                                                            </td>
                                                            <td align="center">
                                            ...
                                            echo "<td colspan=\"6\">No shopper program bins found.</td>";
                                        }
                                        ?>
                                    </tbody>
                                </table>
